...
export async function fetchLatestIssue(): Promise<RawNewsletter> {
  const parser = new Parser();
  const feed = await parser.parseURL(FEED_URL);
  const first = feed.items?.[0];
  if (!first) throw new Error('No items found in feed.');
  return buildNewsletter(first);
}

export async function fetchLatestIssues(count: number): Promise<RawNewsletter[]> {
  const parser = new Parser();
  const feed = await parser.parseURL(FEED_URL);
...
export async function fetchIssue(issueNumber: number): Promise<RawNewsletter> {
  const parser = new Parser();
  const feed = await parser.parseURL(FEED_URL);
  const match = feed.items?.find((i: any) => i.link?.endsWith(`/${issueNumber}`));
  if (!match) throw new Error(`Issue #${issueNumber} not found in feed.`);
  return buildNewsletter(match);
}

export async function fetchLatestIssueNumbers(count: number): Promise<number[]> {
  const parser = new Parser();
  const feed = await parser.parseURL(FEED_URL);
  return (feed.items || [])
    .slice(0, count)
    .map((it: any) => parseInt((it.link || '').split('/').pop() || '0', 10))
    .filter((n: number) => n > 0);
}

function buildNewsletter(item: any): RawNewsletter {
...
function isSponsorSection(section: string): boolean {
  return /sponsor/i.test(section);
}

function extractItems(html: string): { intro: string; items: RawItem[] } {
  ...
  let featuredDescBuffer: string[] = [];
  let lastFeaturedItem: RawItem | null = null;
  ...
  for (const node of root.childNodes) {
    ...
    if (tag === 'ul' || tag === 'ol') {
      ...
      if (lastFeaturedItem && linkBearing.length === 0) continue;
      flushFeaturedDesc();
      lastFeaturedItem = null;
      for (const li of lis) {
        const item = extractItemFromLi(li, currentSection, currentEmoji);
        if (!item) continue;
        if (isYoutube(item.url)) continue;
        if (isSponsorSection(item.section)) continue;
        items.push(item);
      }
    }
  }
...
function extractItemFromLi(li: HTMLElement, section: string, sectionEmoji: string): RawItem | null {
